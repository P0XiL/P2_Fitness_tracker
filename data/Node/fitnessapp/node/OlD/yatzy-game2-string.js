...
function ScoreTable(M){
  ...
  */
  ...
  this.printScores=function(){
    for(let scoreEntry of this.playScore){
      let s="";
      scoreEntry.diceRoll.forEach(d=>s+=d);
      console.log(scoreEntry.roundName+ "\t\t\t"+scoreEntry.score + "\t\t\t"+s);
    };
  };
  ...
  this.playGame=function(roundNo,r){
     let c;
     console.log("executing: " +roundNo+ " "+r);
      //r=roll(this.M);
      //r=[1,5,5,5,2,2,6]; //M=7
      if(this.alreadyPlayed(roundNo)) return;
      c=countDice(r);
      let score=0;

      switch(rounds[roundNo]){
        case "1s": 
          score=1*c[1];
          break;
        case "2s":
      ...
      this.noteScore(roundNo,score,r);
